...
async function loadFollowers(searchTerm = "") {
    ...
    try {
        let url = `${API_URL}?user_id=${userId}&type=following`;
        if (searchTerm) {
    ...
    followers.forEach(follower => {
        ...
        div.innerHTML = `
            <div class="follower-left d-flex align-items-center gap-2">
                <img src="${follower.main_user_image}" alt="" style="border-radius:10px;">
                <div>
                     <a href="profile.html?user_id=${follower.main_user}" class="text-secondary text-decoration-none">
                        <strong>${follower.main_user_username}</strong>
                    </a><br>
                    <span class="muted">Following</span>
                </div>
            </div>
            <button class="btn btn-sm btn-outline-danger btn-remove" data-follower="${follower.main_user}">Unfollow</button>
        `;
...
async function removeFollower(followerUserId) {
    try {
        const res = await fetch(`${API_URL}manage/?main_user=${userId}&follower=${followerUserId}&action=unfollow`, {
            method: "DELETE",
            headers: { "Authorization": `Token ${token}` }
        });
        const data = await res.json();
